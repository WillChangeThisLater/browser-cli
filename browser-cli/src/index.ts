...
`)
  .action(async (selector: string, options: any) => {
    ...
    try {
      await click(session.page, selector, options);
    } finally {
      await session.close();
      process.exit(0);
    }
...
`)
  .action(async (selector: string, text: string, options: any) => {
    ...
    try {
      await type(session.page, selector, text, {
        url: options.url,
        clear: options.clear,
...
`)
  .action(async (path: string, options: any) => {
    ...
    try {
      await screenshot(session.page, path, options);
    } finally {
      await session.close();
      process.exit(0);
    }
...
`)
  .action(async (code: string, options: any) => {
    ...
    try {
      await evalJs(session.page, code, options);
    } finally {
      await session.close();
      process.exit(0);
    }
...
`)
  .action(async (selector: string | undefined, options: any) => {
    ...
    try {
      await inspect(session.page, selector, {
        url: options.url,
        all: options.all,
